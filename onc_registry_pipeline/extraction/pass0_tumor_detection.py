...
class TumorDetector:
    ...
    def _build_running_list_system_prompt(self) -> str:
        return (
            ...
            "   - \"infiltrating ductal carcinoma\", \"invasive lobular "
            "carcinoma\", and \"mixed ductal and lobular carcinoma\" of the "
            "SAME breast around the same date (different histologic components "
            "of one tumor)\n"
            ...
            "9. Differing histologic subtype, grade, or descriptive "
            "granularity for the SAME anatomic site and laterality within the "
            "same diagnostic timeframe indicates ONE primary, not several -- "
            "collapse them into a single entry with the most specific or "
            "representative histology, even when the wording differs "
            "substantially. When in doubt about whether two descriptions at "
            "one site/laterality are one primary or two, MERGE them.\n"
            "10. Do NOT create a separate primary for an abnormality at a site "
            "that is a common METASTATIC or incidental location (e.g. adrenal, "
            "liver, lung nodule, bone, brain, peritoneum, or a non-regional "
            "lymph node) unless the text independently establishes a primary "
            "cancer arising at that site (e.g. a distinct histology with a "
            "documented site of origin). Such lesions are usually metastases "
            "from a known primary or indeterminate findings, and a metastasis "
            "is never a separate primary (see rule 1).\n\n"
