...
1. TNM STAGING: Distinguish between clinical (c) and pathological (p) staging.
   ...
   - Do NOT mix clinical and pathological components. Populate whichever category (clinical, pathological, or both) IS documented; leave a category unknown only when it has no documented staging -- never leave a documented category unknown.
...
10. ABSTENTION vs. NOT APPLICABLE: If a staging item is genuinely not addressed anywhere in the text, use the "unknown" code (e.g., 9 or 99). Reserve the "not applicable" code (88) for cases where AJCC TNM staging does not apply to this cancer's schema (e.g., many hematopoietic/lymphoid neoplasms, or ill-defined/unknown primary sites) -- do NOT use 88 merely because a value was not mentioned. Never invent values unsupported by the text; but if the text DOES state a stage or TNM, you must capture it (see rules 14-15).
11. GRADE: Clinical grade is from biopsy before definitive treatment. Pathological grade is from the surgical specimen.
12. DISTANT METASTASIS REQUIRES CONFIRMATION: Assign distant disease -- Summary Stage 2018 = 7, clinical/pathologic M1, or any Mets at Diagnosis site = 1 (yes) -- ONLY when the text affirmatively establishes metastatic spread (biopsy-proven metastasis, or imaging/clinical documentation that unequivocally describes a lesion as metastatic). Do NOT upstage to distant based on findings described as indeterminate, equivocal, "too small to characterize", "cannot exclude", "concerning/suspicious for", or lesions only flagged for further work-up and not yet confirmed. When metastatic potential is raised but unconfirmed, code distant mets as NOT identified and stage on the confirmed extent of disease (e.g., regional nodal involvement) -- do not let an unconfirmed lesion drive Summary Stage to 7.
13. HEMATOPOIETIC / LYMPHOID NEOPLASMS: Leukemias (acute or chronic, including CLL/SLL involving blood or marrow), multiple myeloma, and other plasma-cell or disseminated bone-marrow/blood neoplasms are SYSTEMIC at diagnosis. For these, code Summary Stage 2018 = 7 (distant); do NOT leave Summary Stage as 9 (unknown) or blank merely because anatomic or TNM staging is not documented. (Lymphomas are staged by extent of involvement: localized single-region disease may be localized, while widespread nodal-both-sides or extranodal disease is distant.)
14. DECOMPOSE STATED STAGING: Whenever the text contains a TNM expression or stage statement -- e.g. "cT3N2cM0", "pT1c N0 M0", "Stage IVA", "AJCC Stage IIIB" -- you MUST populate EVERY component item it supports, not just Summary Stage: the individual T, N, and M items AND the stage group, using the correct clinical vs. pathological category indicated by the prefix (c = clinical, p = pathological; yc/yp = post-neoadjuvant). For example, from "cT3N2cM0": clinical T = cT3 (or 3), clinical N = cN2 (or 2), clinical M = cM0 (or 0). It is an ERROR to quote a TNM string as evidence for one field (such as Summary Stage) while leaving the individual T/N/M items unknown -- the same stated value must flow into each component item it names.
15. STAGE GROUP: If an overall stage group is explicitly stated (e.g. "Stage IIIB"), record it. If T, N, and M are documented but the overall group is not explicitly stated, derive the AJCC stage group for this cancer's schema from the documented T/N/M (and any required factors such as grade or biomarkers) and record it, noting the basis in the evidence quote. Use the registry's expected format: stage group as 0/I/II/III/IV with any subdivision letter (A/B/C), and T/N/M carrying their category prefix.
